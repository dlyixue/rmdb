...
void DiskManager::write_page(int fd, page_id_t page_no, const char *offset, int num_bytes) {
    ...
    off_t currpos;
    currpos = lseek(fd, page_no * PAGE_SIZE,SEEK_SET);
    if(write(fd, offset, num_bytes) !=  num_bytes){
        std::cout << "write_page fail" << std::endl;
...
void DiskManager::read_page(int fd, page_id_t page_no, char *offset, int num_bytes) {
    ...
    if(read(fd, offset, num_bytes) < 0){
        std::cout << "read_page fail" << std::endl;
...
void DiskManager::create_file(const std::string &path) {
    ...
    // 注意不能重复创建相同文件
    // open函数在创建相同文件时不进行操作
    if(open(path.c_str(), O_CREAT) < 0){
        throw FileExistsError("create_file fail");
        std::cout << "create_file fail" << std::endl;
    }
}
...
void DiskManager::destroy_file(const std::string &path) {
    ...
    // 调用unlink()函数
    // 注意不能删除未关闭的文件
    // unlink在删除为关闭文件时，会使其link数-1, 并在close时进行关闭
    if(unlink(path.c_str()) < 0){
        // std::cout << "destroy_file fail "<< path << std::endl;
    }
}
...
int DiskManager::open_file(const std::string &path) {
    ...
    // 打开重复文件会导致多个fd，所以需要取在文件表里查看是否打开
    if(path2fd_.find(path) != path2fd_.end()){
        std::cout << "open_file: file is exist" << std::endl;
        throw FileNotFoundError("open_file fail");
        return 0;
    }
    int fd = open(path.c_str(), O_RDWR);
    if(fd < 0){
        // std::cout << fd <<" open_file fail " << path << std::endl;
        throw FileNotFoundError("open_file fail");
        return 0;
    }
    path2fd_[path] = fd;
...
void DiskManager::close_file(int fd) {
    ...
    if(fd2path_.find(fd) == fd2path_.end()){
        std::cout << "close_file: file is not open" << std::endl;
    }
    if(close(fd) < 0){
        std::cout << "close_file fail" << std::endl;
    }
